...
export default {
  ...
  mutations: {
    ...
    create(state, payload) {
      console.log(state.listResponse)
      state.listResponse.unshift(payload);
    },
    update(state, payload) {
      let index = state.listResponse.findIndex(
    ...
    delete(state, payload) {
      let filteredList = []
      filteredList = state.listResponse.filter(item => !payload.includes(item.code));
      state.listResponse.results = [...filteredList];
    },
    changeStatus(state, payload) {
      state.listResponse.forEach(item => {
    ...
    async list({ commit }, params) {
      ...
      try {
        const response = await axiosBusiness.get(
            `${BACKENDURL}/`)
        commit("setListResponse", response.data.results);
        commit('setTreeNodes', response.data.results)
