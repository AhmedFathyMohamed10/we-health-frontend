...
export default {
  ...
  getters: {
    ...
    getPermission: (state) => state.form.permissions,
  ...
  actions: {
    ...
    async list({ commit }, params) {
      ...
      try {
        const response = await axiosBusiness.get(
          `${BACKENDURL}/?page=${params.pageNumber}&limit=${params.rows}&company_name_ar=${params.company_name_ar}&company_name_en=${params.company_name_en}&group_id=${params.group_id}`
        );
    ...
    async move({ commit }, payload) {
      ...
      try {
        ...
        console.log(error)
        commit('setMoveError', error.response.data)
        return Promise.reject(error)
      }
      finally {
