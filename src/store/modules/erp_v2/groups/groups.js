...
const BACKENDURL = 'erp/client-supplier-group';
...
export default {
  ...
  actions: {
    ...
    async list({ commit }, params) {
      console.log("client supplier groups",params)
      commit('setListLoading', true)
      try {
        const response = await axiosBusiness.get(
            `${BACKENDURL}/?category=${params.category}`)
    ...
    async search({ commit }, payload) {
      ...
      try {
        const response = await axiosBusiness.get(`${BACKENDURL}/search`, {
          params: { name_en: payload.name_en, category:payload.category },
        });
