...
export default {
  ...
  actions: {
    ...
    async list({ commit }, params) {
      ...
      try {
        const response = await axiosBusiness.get(
          `${BACKENDURL}/search-secondary-account?page=${params.pageNumber}&limit=${params.rows}&account_name_en=${params.account_name_en}&final_account=${params.final_account_id}&parent_account=${params.parent_account_id}`
        );
    ...
    async listPrimaryAccount({ commit }, params) {
      ...
      try {
        const response = await axiosBusiness.get(
          `${BACKENDURL}/search-primary-account`
        );
    ...
    async create({ commit }, form) {
      ...
      try {
        ...
        if (response.data.data[0].account_disposition === 1) {
          ...
          console.log('createPrimary Else');
