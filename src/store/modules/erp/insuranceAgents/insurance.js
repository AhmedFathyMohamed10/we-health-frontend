...
const BACKENDURL = 'erp/insurance-agent';
...
export default {
  ...
  actions: {
    ...
    async list({ commit }, params) {
      ...
      try {
        const response = await axiosBusiness.get(`${BACKENDURL}/?page=${params.pageNumber}&limit=${params.rows}&name=${params.name}&mobile=${params.mobile}`);
    ...
    async delete({ commit }, ids) {
      ...
      try {
        const response = await axiosBusiness.post(`${BACKENDURL}/delete`, ids);
        console.log(response);
