...
export default {
  ...
  getters: {
    paymentMethods: state => state.listResponse,
    paymentMethodsLoading: state => state.listLoading,
    paymentMethodsError: state => state.listError,
  },
  ...
  actions: {
    ...
    async list({ commit }) {
      ...
      try {
        const response = await axiosBusiness.get('erp/accounting/invoices/definition/payment-methods');
