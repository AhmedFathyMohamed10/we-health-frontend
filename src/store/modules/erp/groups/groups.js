...
const BACKENDURL = 'erp/clients-groups';
...
export default {
  ...
  actions: {
    ...
    async list({ commit }, params) {
      ...
      try {
        const response = await axiosBusiness.get(
            `${BACKENDURL}/?page=${params.pageNumber}&limit=${params.rows}&name=${params.name}&discountPercentage=${params.discount_percentage}`)
    ...
    async update({ commit }, form) {
      ...
      let data={group:form,token:token}
      commit('setUpsertLoading', true)
      try {
        const {name,discount_percentage,description} = form
        let data =  {name,discount_percentage,description};
