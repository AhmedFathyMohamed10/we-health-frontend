...
const BACKENDURL = "erp/hr-custodies";
...
export default {
  ...
  actions: {
    ...
    async list({ commit }, params) {
      ...
      try {
        const response = await axiosBusiness.get(
          `${BACKENDURL}/?page=${params.pageNumber}&limit=${params.rows}&employee_name=${params.employee_name}&pk=${params.pk}`
        )
    ...
    async update({ commit }, form) {
      ...
      const {title,type, receipt_date, delivery_date, employee} = form
      let data =  {title,type, receipt_date, delivery_date, employee};
      commit('setUpsertLoading', true)
      try {
    ...
    async getTypeOptions({ commit }, params) {
      commit('setTypeOptions', true)
      try {
        const response = await axiosBusiness.get(
          `${BACKENDURL}/get-options`
        )
        commit("setTypeOptions", response.data.type_options);
        commit('setListError', null)
        return Promise.resolve(response.data.type_options)
      } catch (error) {
        console.log(error)
        commit('setListError', error.response.data)
        return Promise.reject(error.response.data)
