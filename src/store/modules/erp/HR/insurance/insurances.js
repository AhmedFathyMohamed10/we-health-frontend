...
const BACKENDURL = "erp/hr-insurances";
...
export default {
  ...
  actions: {
    ...
    async list({ commit }, params) {
      ...
      try {
        const response = await axiosBusiness.get(
          ...
          `${BACKENDURL}/?page=${params.pageNumber}&limit=${params.rows}&policy_title=${params.policy}&employee_name=${params.employee}&pk=${params.pk}`
        )
    ...
    async update({ commit }, form) {
      var token = localStorage.getItem("token");
      const { policy, employee, start_date, end_date, code } = form
      let data =  { policy, employee, start_date, end_date, code };
      commit('setUpsertLoading', true)
      try {
