...
const BACKENDURL = "erp/hr-insurance-policies";
...
export default {
  ...
  actions: {
    ...
    async list({ commit }, params) {
      ...
      try {
        const response = await axiosBusiness.get(
          `${BACKENDURL}/?page=${params.pageNumber}&limit=${params.rows}&title=${params.title}&insurance_agent=${params.insurance_agent}`
        )
    ...
    async update({ commit }, form) {
      var token = localStorage.getItem("token");
      const { title, insurance_agent, type, insurance_amount, code } = form
      let data =  { title, insurance_agent, type, insurance_amount, code };
      commit('setUpsertLoading', true)
      try {
