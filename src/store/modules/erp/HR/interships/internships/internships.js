...
const BACKENDURL = "erp/hr-internships";
...
export default {
  ...
  actions: {
    ...
    async list({ commit }, params) {
      ...
      try {
        const response = await axiosBusiness.get(
          `${BACKENDURL}/?page=${params.pageNumber}&limit=${params.rows}&entity_name=${params.entity_name}&course_name=${params.course_name}&employee_name=${params.employee_name}&department_name=${params.department_name}`
        )
    ...
    async update({ commit }, form) {
      var token = localStorage.getItem("token");
      const {course, employee, start_date, end_date} = form
      let data =  {course, employee, start_date, end_date};
      commit('setUpsertLoading', true)
      try {
