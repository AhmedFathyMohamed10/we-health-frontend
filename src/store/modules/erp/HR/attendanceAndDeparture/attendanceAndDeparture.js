...
const BACKENDURL = "erp/hr/attendance-departure";
    ...
    async list({ commit }, params) {
      ...
      try {
        const response = await axiosBusiness.get(
          `${BACKENDURL}/?page=${params.pageNumber}&limit=${params.rows}&employee=${params.employee}&type=${params.type}`
        );
    ...
    async search({ commit }, payload) {
      ...
      try {
        const response = await axiosBusiness.get(`${BACKENDURL}/search`, {
          params: { name: payload.name, category: payload.category },
        });
        // console.log(response)
