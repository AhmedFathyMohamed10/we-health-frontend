...
export default {
  ...
  actions: { 
    async documentsList({commit},payload) {
      console.log(`from payload category verify ${payload.id}`)
      try {
        commit('setServerLoading', true);
        const response = await axiosBusinessRegistration.get(`list_images/${payload.id}/`);
