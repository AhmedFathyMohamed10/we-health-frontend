<template>
<div class="lensometry">
        <Base>
        <div class="row">
            <div class="col-12 d-flex justify-content-between">
                <h4>{{ $t("lensometry.header") }}</h4>
        ...
        <div class="row my-4">
            ...
            <div class="col-md-6 col-xl-4">
                <div class="form-group">
                    <CustomDropdown
                        v-model="lensometry.lensType"
                        :label="$t('lensometry.lensType.label')"
                        ...
                        <div v-if="v.lensType.other.$error && !v.lensType.other.notContainsHtmlTags" class="invalid-feedback">
                                    {{htmlTagsError($t('lensometry.lensType.label'))}}
                        </div>
                        <div v-else-if="! v.lensType.other.minLength &&  v.lensType.other.$error" class="invalid-feedback">
                                {{minLengthError($t('lensometry.lensType.label'), v.lensType.other.$params.minLength.min)}}
                        </div>
                        <div v-else-if="! v.lensType.other.maxLength &&  v.lensType.other.$error" class="invalid-feedback">
                                {{maxLengthError($t('lensometry.lensType.label'), v.lensType.other.$params.maxLength.max)}}
                        </div>                 
                    </CustomDropdown>
                </div>
            </div>
        ...
        <div class="row">
            <div class="col-12 svg-wrapper">
                <img src="@/assets/mr/ophthalmic/lensometry/layer1.svg" alt="" class="w-100">
            </div>
            <div class="col-12 labels-wrapper">
                <div class="row">
                    <div class="col-6 label-wrapper d-flex justify-content-center"><h3>O.D.</h3></div>
                    <div class="col-6 label-wrapper d-flex justify-content-center"><h3>O.S.</h3></div>
                </div>
            </div>
        </div>            
        <!-- Od Row -->
        <div class="row" >
            ...
            <div class="col-md-3">
                <div class="form-group">
                    <CustomDropMenu 
                        v-model="lensometry.od.spherical"
            ...
            <div class="col-md-3">
                <div class="form-group">
                    <!-- Cylindrical -->
                    <CustomDropMenu 
                        v-model="lensometry.od.cylindrical"
        ...
        <div class="row" >
            ...
            <div class="col-md-3">
                <div class="form-group">
                    <CustomDropMenu 
                        v-model="lensometry.os.spherical"
                        :label="$t('measurementsLabels.spherical')"
                        :options="sphericalOptions"
                        :excludeColumn="0"
                        :invalid="
            ...
            <div class="col-md-3">
                <div class="form-group">
                    <!-- Cylindrical -->
                    <CustomDropMenu 
                        v-model="lensometry.os.cylindrical"
                        :label="$t('measurementsLabels.cylindrical')"
                        :options="cylindricalOptions"
                        :excludeColumn="0"
                        :invalid="
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    <label class="form-label">{{$t('lensometry.note.label')}}</label>
                    <textarea 
                        v-model="lensometry.note" 
                        :placeholder="$t('lensometry.note.placeholder')" 
                        class="form-control"
                        :class="{
                            'is-invalid':
                            (v.note.$error)
                    ...
                    <div v-if="v.note.$error && !v.note.notContainsHtmlTags" class="invalid-feedback">
                            {{htmlTagsError($t('lensometry.note.label'))}}
                    </div>
                    <div v-else-if="! v.note.minLength &&  v.note.$error" class="invalid-feedback">
                            {{minLengthError($t('lensometry.note.label'), v.note.$params.minLength.min)}}
                    </div>
                    <div v-else-if="! v.note.maxLength &&  v.note.$error" class="invalid-feedback">
                            {{maxLengthError($t('lensometry.note.label'), v.note.$params.maxLength.max)}}
                    </div> 
                </div>
            </div>
        </div>            
...
////////////////////////////////
  computed: {
    ...mapGetters("detailsOphthalmic/ophthalmic", ["getLensometry"]),
    lensometry: {
      get() {
        return this.getLensometry;
      },
      set(value) {
        this.$store.dispatch("detailsOphthalmic/ophthalmic/setLensometry", value);
      },
    }
  },
  ///////////////////////////////////
...
//////////////////////////////////////
  ...
  //////////////////////////////
    ...
    copyToOd(){
        this.lensometry.od=JSON.parse(JSON.stringify(this.lensometry.os));
    },
    copyToOs(){
        this.lensometry.os=JSON.parse(JSON.stringify(this.lensometry.od));
    },
