  ...
  <div class="pulse-rate">
    <div class="pulse-rate-header d-flex justify-content-between align-items-center">
      <div class="d-flex gap-2 align-items-center">
        <h4 class="mb-0"><i class="pi pi-eye fs-2 text-success-gradient"></i></h4>
        <h4 class="mb-0"> Inbody </h4>
      </div>
    ...
    <DetailsDialog :display="displayDetailsDialog" @hide="closeDetailsDialog" :header="$t('header.title')" :maximizable="true">
       <div class="row">
        <div class="col-md-12">
          ...
          <div class="">
            <h4><i class="pi  pi-eye  fs-2 text-success-gradient"></i></h4>
          </div>
          <div class="">
            <button class="btn btn-pill btn-secondary-gradient" @click.prevent="toggleView">
              {{ view=='report'?'Chart':'Report' }}
            </button>
          </div>
        </div>
...
<i18n src="@/lang/profile/measurements/Inbody.json"></i18n>
<script>
...
export default {
  ...
  computed:{
    ...mapGetters("measurements/Inbody", {
  ...
  methods: {
    ...
    ...mapActions("measurements/Inbody",
    ...
    resetForm(){
      this.form = {beats:"",comment: ""};
      this.closeForm();
      this.upsertResponse=null;
      this.upsertError=null;
    },
    create(){
      this.$store.dispatch("measurements/Inbody/create", this.form)
      .then((res) => {
        // console.log(res)
        this.resetForm();
    ...
    update(){
      this.$store.dispatch("measurements/Inbody/update", this.form)
      .then((res) => {
        // console.log(res)
        this.resetForm();
        const summery = res.statusText; 
    ...
    deleteRecord() {
      this.$store.dispatch("measurements/Inbody/delete", this.recordId)
      .then((res) => {
        // console.log(res)
        const summery = res.statusText; 
        const details =  res.data.message;
