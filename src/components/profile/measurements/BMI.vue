  ...
  <div class="pulse-rate">
    <div class="pulse-rate-header d-flex justify-content-between align-items-center">
      <div class="d-flex gap-2 align-items-center">
        <h4 class="mb-0"><i class="pi pi-cog fs-2 text-success-gradient"></i></h4>
    ...
    <DetailsDialog :display="displayDetailsDialog" @hide="closeDetailsDialog" :header="$t('header.title')" :maximizable="true">
        <div class="pulse-rate-toggler d-flex gap-3 justify-content-between">
          <div class="">
            <h4><i class="pi pi-cog fs-2 text-success-gradient"></i></h4>
          </div>
          <div class="">
          ...
          <table class="table table-borderless text-nowrap mb-0">
            <tbody>
              ...
              <tr>
                ...
                <td ><span class="result-msg fw-bold text-start text-white px-3 py-1" :style="{ background: gradientBg(getBeatsResult(form.bmi).status) }">{{getBeatsResult(form.bmi).message}}</span></td>
              </tr>
            </tbody>
          </table>
        </div>
...
export default {
  ...
  methods: {
    ...
    create(){
      this.$store.dispatch("measurements/BMI/create", this.form)
      .then((res) => {
        // console.log(res)
        this.resetForm();
    ...
    update(){
      this.$store.dispatch("measurements/BMI/update", this.form)
      .then((res) => {
        // console.log(res)
        this.resetForm();
        const summery = res.statusText; 
    ...
    deleteRecord() {
      this.$store.dispatch("measurements/BMI/delete", this.recordId)
      .then((res) => {
        const summery = res.statusText; 
        const details =  res.data.message;
        this.successToast(summery,details)
