      ...
      <Base>
        ...
        <div class="row">
          <div class="col-12">
            ...
            <div class="p-2">
              <strong>{{ $t("uploadedDocs") }} : </strong
              ><small>{{ recordsToDisplay.length }}</small>
            </div>
          </div>
          <div class="col-12">
            <div class="w-100 d-flex flex-wrap">
              <div
                v-for="(record, index) in recordsToDisplay"
                :key="index"
              ...
              >
                <div class="file-record m-2">
                  <div class="p-2">
                    <DisplayFileBtn
                      :file="record.file"
                      @deleteFileItem="openDeleteDialog(index, record)"
                    ></DisplayFileBtn>
                  </div>
                  <h6 class="p-2">
    ...
    >
      <div class="row">
        <div class="col-md-6 col-xl-4">
          <div class="form-group">
            <label class="form-label">{{
              $t("materialPicture.title.label")
            }}</label>
            <input
              type="text"
              :placeholder="$t('materialPicture.title.placeholder')"
              v-model="rowToAdd.title"
              class="form-control"
              :class="{
                'is-invalid':
                  $v.rowToAdd.title.$invalid &&
                  $v.rowToAdd.title.$anyDirty &&
                  $v.rowToAdd.title.$anyError,
              }"
            />
            <div
            ...
            >
              {{ requiredError($t("materialPicture.title.label")) }}
            </div>
            <div
              v-else-if="
                !$v.rowToAdd.title.notContainsHtmlTags &&
                $v.rowToAdd.title.$error
              "
              class="invalid-feedback"
            >
              {{ htmlTagsError($t("materialPicture.title.label")) }}
            </div>

            <div
              v-else-if="
                !$v.rowToAdd.title.maxLength && $v.rowToAdd.title.$error
              "
              class="invalid-feedback"
            >
              {{
        ...
        <div class="col-md-6 col-xl-4">
          <div class="form-group">
            <label class="form-label">{{
              $t("materialPicture.comment.label")
            }}</label>
            <input
              type="text"
              :placeholder="$t('materialPicture.comment.placeholder')"
              v-model="rowToAdd.comment"
              class="form-control"
              :class="{
                'is-invalid':
                  $v.rowToAdd.comment.$invalid &&
                  $v.rowToAdd.comment.$anyDirty &&
                  $v.rowToAdd.comment.$anyError,
              }"
            />
            <div
              v-if="
              ...
              class="invalid-feedback"
            >
              {{ htmlTagsError($t("materialPicture.comment.label")) }}
            </div>
        ...
        <div class="col-md-6 col-xl-4" v-if="inputFile">
          <div class="form-group">
            <label class="form-label">{{
              $t("materialPicture.file.label")
            }}</label>
            <DisplayFileBtn
              :file="inputFile"
              @deleteFileItem="closeAddDialog"
            ></DisplayFileBtn>
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...mapGetters("erp/ProductAndService/productAndService/product", {
      getMaterialPicture: "getMaterialPicture",
    }),
    form: {
      get() {
        return this.getMaterialPicture;
      },
      set(value) {
        this.setMaterialPicture(value);
      },
    },
  },
  data() {
  ...
  mounted() {
    if (this.isUpdating) {
      let records = [...this.form];
      records.forEach((record) => {
        const { id, code, title, comment } = record;
        let file = this.handleFile(record.file);
        this.recordsToDisplay.push({ title, file, id, code, comment });
      });
    }
  },
  methods: {
    ...
    addRow() {
      ...
      if (
        this.inputFile.base64 &&
        this.inputFile.validExtension &&
        this.inputFile.validSize
      ) {
    ...
    openDeleteDialog(index, record) {
        ...
        this.indexOfDeletedPicture = index;
        this.dataObject = record;
    },
    closeDeleteDialog() {
      this.displayDeleteModal = false;
      this.indexOfDeletedPicture = null;
    },
    deleteRow() {
      this.form.splice(this.indexOfDeletedPicture, 1);
      this.recordsToDisplay.splice(this.indexOfDeletedPicture, 1);
      this.dataOfIndexDeleted.push(this.dataObject);
      this.setDeletedMaterialPicture(this.dataOfIndexDeleted);
