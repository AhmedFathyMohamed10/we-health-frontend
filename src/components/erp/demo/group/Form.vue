        ...
        <div class="row">
          ...
          <div class="col-md-6">
            <div class="form-group">
              ...
              /> -->
              <!-- <div v-if="!$v.form.parent.numeric && $v.form.parent.$error" class="invalid-feedback">
                {{numericError($t('form.parent.label'))}}
              </div> -->
            </div>
          </div>
          <div class="col-md-6">
          ...
          <div class="col-md-6">
            <div class="form-group">
              <label for="" class="form-label">{{$t("form.group_serial.label")}}</label>
              <input
                type="text"
                v-model="form.group_serial"
                :placeholder="$t('form.group_serial.placeholder')"
                class="form-control"
                :class="{'is-invalid': $v.form.group_serial.$error}"
              />
              <div v-if="!$v.form.group_serial.required && $v.form.group_serial.$error " class="invalid-feedback">
                {{ requiredError($t("form.group_serial.label")) }}
              </div>
              <div v-else-if="!$v.form.group_serial.minLength && $v.form.group_serial.$error" class="invalid-feedback" >
                {{minLengthError($t("form.group_serial.label"),$v.form.group_serial.$params.minLength.min)}}
              </div>
              <div v-else-if=" !$v.form.group_serial.maxLength && $v.form.group_serial.$error " class="invalid-feedback" >
                {{ maxLengthError($t("form.group_serial.label"),$v.group.group_serial.$params.maxLength.max)}}
              </div>
              <div v-else-if="! $v.form.group_serial.notContainsHtmlTags &&  $v.form.group_serial.$error" class="invalid-feedback">
                {{htmlTagsError($t('form.group_serial.label'))}}
              </div>
            </div>
          </div>
        </div>
