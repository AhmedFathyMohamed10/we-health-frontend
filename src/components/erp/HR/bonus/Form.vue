    ...
    >
      ...
      <div class="row">
        ...
        <div class="col-md-6">
          ...
          <div
          v-else-if="
            !$v.form.reason.maxLength && $v.form.reason.$error
          "
          class="invalid-feedback"
        >
          {{  maxLengthError(
...
<i18n src="@/lang/erp/hr/bonus/bonus_groups.json"></i18n>
<script>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("erp/hr/bonus_groups", {
    ...
    // },
    groupsOptions() {
      if (this.$store.getters["erp/hr/bonus_groups/getListResponse"])
        return this.$store.getters["erp/hr/bonus_groups/getListResponse"].results;
      else {
        return [];
      }
    },
  ...
  created() {
    ...
    if (!this.groupsOptions.Length > 0) {
      ...
      this.$store.dispatch("erp/hr/bonus_groups/list", params);
    }
  ...
  methods: {
    ...mapActions("erp/hr/bonus_groups", [
      "setUpsertResponse",
      "setUpsertError",
      "setDeleteResponse",
      "setDeleteError",
    ...
    saveForm() {
      ...
      if (!this.$v.form.$invalid) {
        // let id = !this.form.id ? this.form.employee.id : this.form.employee
      //   let id = ""
      //   if (typeof this.form.employee === "object") {
      //     console.log("object")
      //      id = this.form.employee.id;
      // } else if (typeof this.form.employee === "string") {
      //    id = this.form.employee_id
      // } 
      //   console.log()
    ...
    create() {
      this.$store
        .dispatch("erp/hr/bonus_groups/create", this.groupForm)
        .then((res) => {
          this.resetForm();
    ...
    update() {
      this.$store
        .dispatch("erp/hr/bonus_groups/update", this.groupForm)
        .then((res) => {
          this.resetForm();
          const summery = res.statusText;
          const details = res.data.message;
