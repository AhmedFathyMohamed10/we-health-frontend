            ...
            <div class="table-responsive">
              <table class="table table-sm text-center">
                <thead>
                  ...
                  <draggable v-model="statuses" @end="drag=false" element="tbody">
                      <tr v-for="(status, index) in statuses" :key="index">
                        ...
                        <td class="align-middle">
                          <span>
                            <a
                              ...
                              @click="openDetailsDialog(status)"
                              >{{ status.name }}</a
                            >
                          </span>
                        </td>
                        <td class="align-middle">
                          ...
                          >
                            <EditButton
                              class="me-2"
                              @edit="openUpdate(status)"
                            ></EditButton>
                            <DeleteButton
                              @delete="openDeleteDialogSingle(status.code,4)"
                            ></DeleteButton>
                          </div>
                        </td>
                      </tr>
...
<i18n src="@/lang/erp/management/tasks/status.json"></i18n>
<script>
import Base from "@/components/global/utilities/Base.vue";
import NoResults from "@/components/global/utilities/NoResults.vue";
import EditButton from "@/components/global/utilities/EditButton.vue";
import DeleteButton from "@/components/global/utilities/DeleteButton.vue";
import Form from "@/components/erp/management/tasks/status/Form.vue";
...
export default {
  ...
  methods: {
    ...
    selectAllRecords() {
      ...
      if (!this.selectAll) {
        for (let i in this.statuses) {
          this.selected.push(this.statuses[i].code);
        }
      }
    },
    ////////// UPSERT ///////////////////
    ...
    update() {
      this.$store
        .dispatch("erp/management/tasks/status/update", this.form)
        .then((res) => {
          this.resetForm();
          const summery = res.statusText;
          const details = res.data.message;
    ...
    deleteRecords() {
      this.$store
        .dispatch("erp/management/tasks/status/changeStatus", {action: this.statusAction,selected: this.selected})
        .then((res) => {
    ...
    openDetailsDialog(record) {
      console.log(record);
      const { name, updated_at } = record;
      this.recordDetails = { name, updated_at };
      this.displayDetailsDialog = true;
    },
    list() {
      this.$store.dispatch("erp/management/tasks/status/list", this.filterParams).then(() => {
