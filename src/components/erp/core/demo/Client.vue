<template>
  <div class="Client">
                ...
                <table class="table table-sm text-center">
                  <thead>
                    <tr>
                      ...
                      <th scope="col">{{ $t("form.client_serial.label") }}</th>
                  ...
                  <tbody>
                    <tr v-for="(client, index) in client" :key="index">
                      ...
                      <td class="align-middle fw-bold">
                        <span>{{ client.group_id }}</span>
                      </td>
                      <td class="align-middle fw-bold">
                        <span>{{ client.client_serial }}</span>
                      </td>
    ...
    <Form v-model="form" :group_id="group_id" :isLoading="upsertLoading" :displayForm="displayForm" @close="resetForm" @cancel="resetForm" @save="upsert" :error="upsertError" :header="isUpdating?$t('dialog.header.edit'):$t('dialog.header.new')"> 
    </Form>
    <EditDialog 
...
import Form from '@/components/erp/demo/client/Form.vue';
...
export default {
  ...
  methods: {
    ...
    selectAllRecords() {
      ...
      if (!this.selectAll) {
        for (let i in this.client) {
          this.selected.push(this.client[i].id);
        }
      }
    },
    ////////// UPSERT ///////////////////
    ...
    create(){
      ...
      this.$store.dispatch("erp/client/create", this.form)
      .then((res) => {
        this.resetForm();
    ...
    update(){
      ...
      this.$store.dispatch("erp/client/update", this.form)
      .then((res) => {
        this.resetForm();
        const summery = res.statusText; 
        const details =  res.data.message;
    ...
    changeStatus() {
        this.$store.dispatch("erp/client/changeStatus",{action:this.statusAction,selected:this.selected})
        .then((res) => {
    ...
    deleteRecords() {
      this.$store.dispatch("erp/client/delete", this.selected)
      .then((res) => {
