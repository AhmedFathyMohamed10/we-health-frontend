          ...
          <div class="col-12">
            <div class="table-responsive">
              <table class="table table-sm text-center">
                ...
                <tbody>
                  <tr v-for="(account, index) in sortedData" :key="index">
...
import Form from "@/components/erp/accounts/account/Form.vue";
...
export default {
  ...
  methods: {
    ...mapActions("erp/accounts/account", [
      "setUpsertResponse",
      "setUpsertError",
      "setDeleteResponse",
      "setDeleteError",
    ...
    openUpdate(item) {
      if (item.parent_account_data) {
        item["parent_account"] = item.parent_account_data;
      } else {
        item["parent_account"] = item.parent_account_id;
      }
      const {
        id,
        code,
    ...
    create() {
      this.$store
        .dispatch("erp/accounts/account/create", this.form)
        .then((res) => {
          this.resetForm();
    ...
    update() {
      this.$store
        .dispatch("erp/accounts/account/update", this.form)
        .then((res) => {
          this.resetForm();
          const summery = res.statusText;
    ...
    deleteRecords() {
      this.$store
        .dispatch("erp/accounts/account/changeStatus", {
    ...
    list() {
      this.$store
        .dispatch("erp/accounts/account/list", this.filterParams)
        .then(() => {
