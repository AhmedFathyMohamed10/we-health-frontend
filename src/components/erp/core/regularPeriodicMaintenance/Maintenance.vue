              ...
              <table class="table table-sm text-center">
                <thead>
                  <tr>
                    ...
                    <th scope="col">{{ $t("form.cost.label") }}</th>
                ...
                <tbody>
                  <tr v-for="(maintenance, index) in maintenances" :key="index">
                    ...
                    <td class="align-middle fw-bold">
                      <span>{{ maintenance.group_name }}</span>
                    </td>
                    <td class="align-middle fw-bold">
                          <span>{{ maintenance.cost }}</span>
                        </td>
                    <td class="align-middle">
                      <span v-if="maintenance.period.value">
                        {{ maintenance.period.value }} /
                      </span>
                      <span v-if="maintenance.period.selection">
                        {{ findLocaleOption(periodOptions, maintenance.period.selection) }}
                      </span>
                    </td>
                    <td class="align-middle">
                      <div
                      ...
                      >
                        <EditButton
                          class="me-2"
                          @edit="openUpdate(maintenance)"
                        ></EditButton>
                        <DeleteButton
                          @delete="openDeleteDialogSingle(maintenance.code, 4)"
                        ></DeleteButton>
                      </div>
                    </td>
                  </tr>
              ...
              <tbody>
                <tr v-for="(value, key) in recordDetails" :key="key">
                  ...
                  <td class="">
                    ...
                    <span v-if="key != 'updated_at' && key != 'period'">{{value}}</span>
...
import Form from "@/components/erp/regularPeriodicMaintenance/maintenance/Form.vue";
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("erp/regularPeriodicMaintenance/Maintenance", {
  ...
  methods: {
    ...mapActions("erp/regularPeriodicMaintenance/Maintenance", [
      "setUpsertResponse",
      "setUpsertError",
      "setDeleteResponse",
      "setDeleteError",
    ...
    selectAllRecords() {
      ...
      if (!this.selectAll) {
        for (let i in this.maintenances) {
          this.selected.push(this.maintenances[i].code);
        }
      }
    },
    ...
    openUpdate(item) {
      ...
      // this.form = { ...newItem };
      const { id, code, name, period, cost, maintenanceGroup, updated_at } = newItem;
      this.form = {
        id,
        code,
        name,
    ...
    create() {
      this.$store
        .dispatch("erp/regularPeriodicMaintenance/Maintenance/create", this.form)
        .then((res) => {
          this.resetForm();
    ...
    update() {
      this.$store
        .dispatch("erp/regularPeriodicMaintenance/Maintenance/update", this.form)
        .then((res) => {
          this.resetForm();
          const summery = res.statusText;
    ...
    deleteRecords() {
      this.$store
        .dispatch("erp/regularPeriodicMaintenance/Maintenance/changeStatus", {
    ...
    list() {
      this.$store.dispatch("erp/regularPeriodicMaintenance/Maintenance/list", this.filterParams).then(() => {
