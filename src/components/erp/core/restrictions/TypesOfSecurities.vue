<template>
    <div class="Group-assets">
      ...
      <div class="search-wrapper my-2">
        <Base>
          <div class="row">
            <div class="col-12">
              <h4>{{ $t("typesOfSecurities.header") }}</h4>
            </div>
          </div>
          <div class="row">
                ...
                <table class="table table-sm text-center">
                  <thead>
                    <tr>
                      ...
                      <th scope="col">{{ $t("form.checkStorageAccount.label") }}</th>
                      <th scope="col">{{ $t("form.bankCashAccount.label") }}</th>
                      <th scope="col">{{ $t("table.thead.controls") }}</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr v-for="(typesOfSecuritie, index) in typesOfSecurities" :key="index">
                      <td class="align-middle">
                        <input
                          type="checkbox"
                          :value="typesOfSecuritie.code"
                          v-model="selected"
                          :binary="true"
                        />
                      </td>
                      ...
                      <td class="align-middle">
                        <span>
                          <a
                            ...
                            @click="openDetailsDialog(typesOfSecuritie)"
                            >{{ typesOfSecuritie.name }}</a
                          >
                        </span>
                      </td>
                      ...
                      <td class="align-middle fw-bold">
                          <span>
                              {{ findLocaleOption(typeOption, typesOfSecuritie.type) }}
                            </span>
                        </td>
                        <td class="align-middle fw-bold">
                          <span>{{ typesOfSecuritie.checkStorageAccount_data.name }}</span>
                        </td>
                        <td class="align-middle fw-bold">
                          <span>{{ typesOfSecuritie.bankCashAccount_data.name }}</span>
                        </td>
                      <td class="align-middle">
                        <div
                        ...
                        >
                          <EditButton
                            class="me-2"
                            @edit="openUpdate(typesOfSecuritie)"
                          ></EditButton>
                          <DeleteButton
                            @delete="openDeleteDialogSingle(typesOfSecuritie.code, 4)"
                          ></DeleteButton>
                        </div>
                      </td>
                    </tr>
              ...
              <table class="table table-borderless text-nowrap mb-0">
                <tbody>
                  <tr v-for="(value, key) in recordDetails" :key="key">
                    <td class="">
                      {{ $t(`typesOfSecurities.key.${key}`) }}
                    </td>
                    <td class="">
                      <span v-if="!value" class="text-muted">------</span>
                      <span
                        v-if="key != 'updated_at' && key!='type' && key != 'checkStorageAccount' && key != 'bankCashAccount' "
                        >{{ value }}</span
                      >

                      <span v-if=" key == 'checkStorageAccount' || key == 'bankCashAccount' "
                      >{{ value.name }} 
                    </span>
  ...
  import Form from "@/components/erp/restrictions/typesOfSecurities/Form.vue";
  ...
  export default {
    ...
    data() {
      return {
        ...
        typeOption: [
        { en: "Receive", ar: "قبض", id: "1" },
        { en: "Payment", ar: "دفع", id: "2" },
        { en: "Payment of checks", ar: "دفع شيكات", id: "3" },
        { en: "Receiving checks", ar: "قبض شيكات", id: "4" },
    ...
    computed: {
      ...
      ...mapGetters("erp/Restrictions/TypesOfSecurities", {
    ...
    mounted() {
      if (!this.typesOfSecurities.length > 0) {
        this.list();
      }
    },
    methods: {
      ...mapActions("erp/Restrictions/TypesOfSecurities", [
        "setUpsertResponse",
        "setUpsertError",
        "setDeleteResponse",
        "setDeleteError",
      ...
      selectAllRecords() {
        this.selected = [];
        if (!this.selectAll) {
          for (let i in this.typesOfSecurities) {
            this.selected.push(this.typesOfSecurities[i].code);
          }
        }
      },
      ...
      openUpdate(item) {
        ...
        const {
            ...
            name,
            type,
            checkStorageAccount = item.checkStorageAccount_data,
            bankCashAccount = item.bankCashAccount_data,
            updated_at
        } = item;
        this.form = {
            id,
      ...
      create() {
        this.$store
          .dispatch("erp/Restrictions/TypesOfSecurities/create", this.form)
          .then((res) => {
            this.resetForm();
      ...
      update() {
        this.$store
          .dispatch("erp/Restrictions/TypesOfSecurities/update", this.form)
          .then((res) => {
            this.resetForm();
            const summery = res.statusText;
      ...
      deleteRecords() {
        this.$store
          .dispatch("erp/Restrictions/TypesOfSecurities/changeStatus", {"action":this.action,"selected":this.selected})
          .then((res) => {
      ...
      openDetailsDialog(record) {
        ...
        const {
            name,
            type,
            checkStorageAccount = record.checkStorageAccount_data,
            bankCashAccount = record.bankCashAccount_data,
            updated_at
        } = record;
        this.recordDetails = {
            name,
      ...
      list() {
        this.$store.dispatch("erp/Restrictions/TypesOfSecurities/list", this.filterParams).then(() => {
