<template>
          ...
          <div class="row">
            <div class="col-md-5 col-xxl-3">
              <div class="form-group">
                <label class="form-label"> {{ $t("form.number.label") }} </label>
                <span
                  :class="[
                ...
                <table class="table table-sm text-center">
                  <thead>
                    <tr>
                      ...
                      <th scope="col">{{ $t("form.time_date.label") }}</th>
                      <th scope="col">{{ $t("form.type_of_restriction.label") }}</th>
                  ...
                  <tbody>
                    ...
                    <td class="align-middle">
                      {{  $moment(bond.time_date).format("YYYY-MM-DD HH:MM A") }}
                    </td>
                    <td class="align-middle">
                      {{ bond.type_of_restriction_bondName }}
                    </td> 
                      <td class="align-middle">
                        <div
                        ...
                        >
                          <EditButton
                            class="me-2"
                            @edit="openUpdate(bond)"
                          ></EditButton>
                          <DeleteButton
                            @delete="openDeleteDialogSingle(bond.code, 4)"
                          ></DeleteButton>
                        </div>
                      </td>
                    </tr>
                ...
                <tbody>
                  <tr v-for="(value, key) in recordDetails" :key="key">
                    ...
                    <td class="">
                      ...
                      <span
                        v-if="key != 'updated_at' &&key != 'time_date'"
                        >{{ value }}</span
                      >
                      <span v-if="key == 'updated_at' ||key == 'time_date'">{{
  ...
  import Form from "@/components/erp/restrictions/bonds/Form.vue";
  ...
  export default {
    ...
    computed: {
      ...
      ...mapGetters("erp/Restrictions/Bond", {
    ...
    methods: {
      ...mapActions("erp/Restrictions/Bond", [
        "setUpsertResponse",
        "setUpsertError",
        "setDeleteResponse",
        "setDeleteError",
      ...
      openUpdate(item) {
      //   item["time_date"] = this.$moment(item.time_date).format(
      //   "MM/DD/YYYY h:mm A"
      // );
         const {
          id,
          ...
          number,
          value,
          type_of_restriction=item.type_of_restriction_id,
          time_date,
          updated_at,
        } = item;
        this.form = {
          ...
          type_of_restriction,
          value,
          time_date : this.$moment(item.time_date).format("YYYY-MM-DD HH:MM"),
          // time_date : this.$moment(item.time_date).format("YYYY-MM-DD HH:MM A"),
          updated_at,
        };
      ...
      create() {
        this.form.time_date=this.$moment(this.form.time_date).format("YYYY-MM-DD HH:MM");
        this.$store
          .dispatch("erp/Restrictions/Bond/create", this.form)
          .then((res) => {
            this.resetForm();
      ...
      update() {
        this.form.time_date=this.$moment(this.form.time_date).format("YYYY-MM-DD HH:MM");
        this.$store
          .dispatch("erp/Restrictions/Bond/update", this.form)
          .then((res) => {
            this.resetForm();
            const summery = res.statusText;
      ...
      deleteRecords() {
        this.$store
          .dispatch("erp/Restrictions/Bond/changeStatus", {"action":this.actionStatus,"selected":this.selected})
          .then((res) => {
      ...
      openDetailsDialog(record) {
        ...
        this.recordDetails = {
            ...
            value,
            time_date,
            type_of_restriction:record.type_of_restriction_bondName,
            updated_at,
        };
        this.displayDetailsDialog = true;
      },
      list() {
        this.$store.dispatch("erp/Restrictions/Bond/list", this.filterParams).then(() => {
