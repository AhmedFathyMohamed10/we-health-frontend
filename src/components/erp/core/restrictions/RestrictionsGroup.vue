<template>
    <div class="Group-restrictions">
      ...
      <div class="search-wrapper my-2">
        <Base>
          <div class="row">
            <div class="col-12">
              <h4>{{ $t("restrictionsGroup.header") }}</h4>
            </div>
          </div>
          <div class="row">
            ...
            <div class="col-12">
              <div class="table-responsive">
                <table class="table table-sm text-center">
                  ...
                  <tbody>
                    <tr v-for="(restrictionsGroup, index) in restrictionsGroups" :key="index">
                      <td class="align-middle">
                        <input
                          type="checkbox"
                          :value="restrictionsGroup.code"
                          v-model="selected"
                          :binary="true"
                        />
                      </td>
                      ...
                      <td class="align-middle">
                        <span>
                          <a
                            ...
                            @click="openDetailsDialog(restrictionsGroup)"
                            >{{ restrictionsGroup.name }}</a
                          >
                        </span>
                      </td>
                      <td class="align-middle">
                        ...
                        >
                          <EditButton
                            class="me-2"
                            @edit="openUpdate(restrictionsGroup)"
                          ></EditButton>
                          <DeleteButton
                            @delete="openDeleteDialogSingle(restrictionsGroup.code, 4)"
                          ></DeleteButton>
                        </div>
                      </td>
                    </tr>
  ...
  import Form from "@/components/erp/restrictions/restrictionsGroup/Form.vue";
  ...
  export default {
    ...
    mounted() {
      if (!this.restrictionsGroups.length > 0) {
        this.list();
      }
    },
    methods: {
      ...mapActions("erp/Restrictions/RestrictionsGroup", [
        "setUpsertResponse",
        "setUpsertError",
        "setDeleteResponse",
        "setDeleteError",
      ...
      selectAllRecords() {
        this.selected = [];
        if (!this.selectAll) {
          for (let i in this.restrictionsGroups) {
            this.selected.push(this.restrictionsGroups[i].code);
          }
        }
      },
      ...
      create() {
        this.$store
          .dispatch("erp/Restrictions/RestrictionsGroup/create", this.form)
          .then((res) => {
            this.resetForm();
      ...
      update() {
        this.$store
          .dispatch("erp/Restrictions/RestrictionsGroup/update", this.form)
          .then((res) => {
            this.resetForm();
            const summery = res.statusText;
      ...
      deleteRecords() {
        this.$store
          .dispatch("erp/Restrictions/RestrictionsGroup/changeStatus", {"action":this.actionStatus,"selected":this.selected})
          .then((res) => {
      ...
      list() {
        this.$store.dispatch("erp/Restrictions/RestrictionsGroup/list", this.filterParams).then(() => {
