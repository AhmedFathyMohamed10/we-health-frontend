          ...
          <div class="col-12">
            <div class="table-responsive">
              <table class="table table-sm text-center">
                ...
                <tbody>
                  <tr v-for="(agent, index) in insuranceAgents" :key="index">
...
export default {
  ...
  mounted() {
    if (!this.insuranceAgents.length > 0) {
      this.list();
    }
  },
  methods: {
    ...
    selectAllRecords() {
      this.selected = [];
      if (!this.selectAll) {
        for (let i in this.insuranceAgents) {
          this.selected.push(this.insuranceAgents[i]);
        }
      }
    },
    ////////// UPSERT ///////////////////
    ...
    create() {
      this.$store
        .dispatch("erp/insuranceAgents/create", this.form)
        .then((res) => {
          this.resetForm();
          // console.log(res)
    ...
    update() {
      this.$store
        .dispatch("erp/insuranceAgents/update", this.form)
        .then((res) => {
          this.resetForm();
          const summery = res.statusText;
          const details = res.data.message;
    ...
    changeStatus() {
      this.$store
        .dispatch("erp/insuranceAgents/changeStatus", {
    ...
    deleteRecords() {
      this.statusAction = 4;
      this.$store
        .dispatch("erp/insuranceAgents/delete", this.selected)
        .then((res) => {
    ...
    openDetailsDialog(record) {
      const { name, mobile, email, website, status, updated_at } = record;
      this.recordDetails = { name, mobile, email, website, status, updated_at };
      this.displayDetailsDialog = true;
    },
    list() {
      this.$store.dispatch("erp/insuranceAgents/list", this.filterParams).then(() => {
