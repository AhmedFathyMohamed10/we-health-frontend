              ...
              <table class="table table-sm text-center">
                <thead>
                  <tr>
                    ...
                    <th scope="col">{{ $t("form.asset.label") }}</th>
                    <th scope="col">{{ $t("form.total_cost.label") }}</th>
                    <th scope="col">{{ $t("form.operation.label") }}</th>

                    <th scope="col">{{ $t("table.thead.controls") }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="(destruction, index) in destructions" :key="index">
                    <td class="align-middle">
                      <input
                        type="checkbox"
                        :value="destruction.code"
                        v-model="selected"
                        :binary="true"
                      />
                    </td>
                    ...
                    <td class="align-middle">
                      <span>
                        <a
                          ...
                          @click="openDetailsDialog(destruction)"
                          >{{ destruction.asset_name }}</a
                        >
                      </span>
                    </td>
                    <td class="align-middle fw-bold">
                      <span>{{ destruction.total_cost }}</span>
                    </td>
                    <td class="align-middle fw-bold">
                      <span>{{
                        findLocaleOption(
                    ...
                    <td class="align-middle">
                      ...
                      >
                        <EditButton
                          class="me-2"
                          @edit="openUpdate(destruction)"
                        ></EditButton>
                        <DeleteButton
                          @delete="openDeleteDialogSingle(destruction.code, 4)"
                        ></DeleteButton>
                      </div>
                    </td>
                  </tr>
      ...
      <div class="row">
        <div class="col-md-12">
            ...
            <table class="table table-borderless text-nowrap mb-0">
              <tbody>
                <tr v-for="(value, key) in recordDetails" :key="key">
                  <td v-if="key !== 'detailed_cost' " class="">
                    {{ $t(`destruction.key.${key}`) }}
                  </td>
                  <td class="">
          ...
          <div
            ...
            v-if="recordDetails && recordDetails.detailed_cost.length > 0"
          >
            <div class="row my-2">
              <div class="col-12">
                <div class="table-responsive">
                  <h4 class="text-center text-danger fw-bold my-4">
                    {{ $t(`destruction.key.detailed_cost`) }}
                  </h4>
                  ...
                  <table class="table table-sm text-center">
                    ...
                    <tbody>
                      <tr
                        v-for="(val, index) in recordDetails.detailed_cost"
                        :key="index"
                      >
                        <td class="align-middle">
                          {{ parseInt(index) + 1 }}
...
import Form from "@/components/erp/assets/destruction/Form.vue";
...
export default {
  ...
  data() {
    return {
      ...
      isUpdating: false,
      operationOptions: [
        { en: "Maintenance", ar: "صيانه", id: 1 },
        { en: "change", ar: "تغيير", id: 2 },
      ],
      ////////////////////
  ...
  computed: {
    ...
    ...mapGetters("erp/Assets/Destruction", {
    ...
    filterParams() {
      return {
        ...
        asset: this.isSearching ? this.searchQuery.asset : "",
      };
    },
  },
  mounted() {
    if (!this.destructions.length > 0) {
      this.list();
    }
  },
  methods: {
    ...mapActions("erp/Assets/Destruction", [
      "setUpsertResponse",
      "setUpsertError",
      "setDeleteResponse",
      "setDeleteError",
    ...
    selectAllRecords() {
      this.selected = [];
      if (!this.selectAll) {
        for (let i in this.destructions) {
          this.selected.push(this.destructions[i].code);
        }
      }
    },
    ////////// UPSERT ///////////////////
    ...
    create() {
      this.$store
        .dispatch("erp/Assets/Destruction/create", this.form)
        .then((res) => {
          this.resetForm();
          // console.log(res)
    ...
    update() {
      this.$store
        .dispatch("erp/Assets/Destruction/update", this.form)
        .then((res) => {
          this.resetForm();
          const summery = res.statusText;
          const details = res.data.message;
    ...
    deleteRecords() {
      this.$store
        .dispatch("erp/Assets/Destruction/changeStatus", {
    ...
    list() {
      this.$store.dispatch("erp/Assets/Destruction/list", this.filterParams).then(() => {
