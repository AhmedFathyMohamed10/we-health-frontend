  ...
  <div class="product-rating">
    ...
    <div class="search-wrapper my-2">
      <Base>
        <div class="row">
          <div class="col-12">
            <h4>{{ $t("productClassification.header") }}</h4>
          </div>
        </div>
        <div class="row">
              ...
              <table class="table table-sm text-center">
                <thead>
                  <tr>
                    ...
                    <th scope="col">{{ $t("form.type_product.label") }}</th>
                ...
                <tbody>
                  <tr
                    v-for="(
                      productClassification, index
                    ) in productClassifications"
                    :key="index"
                  >
                    <td class="align-middle">
                      <input
                        type="checkbox"
                        :value="productClassification.code"
                        v-model="selected"
                        :binary="true"
                      />
                    </td>
                    ...
                    <td class="align-middle">
                      <span>
                        <a
                          ...
                          @click="openDetailsDialog(productClassification)"
                          >{{ productClassification.name }}</a
                        >
                      </span>
                    </td>
                    <td class="align-middle fw-bold">
                      <span>{{
                        findLocaleOption(
                          typeProductOption,
                          productClassification.type_product
                        )
                      }}</span>
                    </td>
                    <td class="align-middle">
                      ...
                      >
                        <EditButton
                          class="me-2"
                          @edit="openUpdate(productClassification)"
                        ></EditButton>
                        <DeleteButton
                          @delete="
            ...
            <table class="table table-borderless text-nowrap mb-0">
              <tbody>
                <tr v-for="(value, key) in recordDetails" :key="key">
                  <td class="">
                    {{ $t(`productClassification.key.${key}`) }}
                  </td>
                  <td class="">
                    <span v-if="!value" class="text-muted">------</span>
                    <span v-if="key !== 'type_product'">{{ value }}</span>
                    <div v-if="key == 'type_product'">
                      <span>
                        {{ findLocaleOption(typeProductOption, value) }}
                      </span>
                    </div>
                  </td>
                </tr>
...
<i18n src="@/lang/erp/productAndService/productClassification.json"></i18n>
<script>
import Base from "@/components/global/utilities/Base.vue";
import NoResults from "@/components/global/utilities/NoResults.vue";
import EditButton from "@/components/global/utilities/EditButton.vue";
import DeleteButton from "@/components/global/utilities/DeleteButton.vue";
import Form from "@/components/erp/productAndServices/productClassification/Form.vue";
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("erp/ProductAndService/productClassification", {
  ...
  methods: {
    ...mapActions("erp/ProductAndService/productClassification", [
      "setUpsertResponse",
      "setUpsertError",
      "setDeleteResponse",
      "setDeleteError",
    ...
    selectAllRecords() {
      this.selected = [];
      if (!this.selectAll) {
        for (let i in this.productClassifications) {
          this.selected.push(this.productClassifications[i].code);
        }
      }
    },
    ...
    create() {
      this.$store
        .dispatch(
          "erp/ProductAndService/productClassification/create",
          this.form
        )
        .then((res) => {
          this.resetForm();
    ...
    update() {
      this.$store
        .dispatch(
          "erp/ProductAndService/productClassification/update",
          this.form
        )
        .then((res) => {
          this.resetForm();
    ...
    deleteRecords() {
      this.$store
        .dispatch("erp/ProductAndService/productClassification/changeStatus", {
    ...
    openDetailsDialog(record) {
      console.log(record);
      const { name, type_product, description, updated_at } = record;
      this.recordDetails = { name, type_product, description, updated_at };
      this.displayDetailsDialog = true;
    },
    list() {
      this.$store
        .dispatch(
          "erp/ProductAndService/productClassification/list",
          this.filterParams
        )
        .then(() => {
