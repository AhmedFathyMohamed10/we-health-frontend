      ...
      <Base>
        <div v-if="listLoading">
          <DataTable :value="[{}, {}, {}]">
            ...
            <Column field="MainCategroy" header="MainCategroy">
              <template #body>
                <Skeleton></Skeleton>
              </template>
            </Column>
                    ...
                    <td class="align-middle">
                      <span>
                        <a
                          ...
                          @click="openDetailsDialog(subCategory)"
                          >{{ subCategory.name }}</a
                        >
                      </span>
                    </td>
                    <td class="align-middle fw-bold">
                      <span>{{ subCategory.main_category_name }}</span>
                    </td>
                    <td class="align-middle">
                      <div
                      ...
                      >
                        <EditButton
                          class="me-2"
                          @edit="openUpdate(subCategory)"
                        ></EditButton>
                        <DeleteButton
                          @delete="openDeleteDialogSingle(subCategory)"
                        ></DeleteButton>
                      </div>
                    </td>
                  </tr>
...
<i18n src="@/lang/erp/productAndService/SubCategory"></i18n>
<script>
import Base from "@/components/global/utilities/Base.vue";
import NoResults from "@/components/global/utilities/NoResults.vue";
import EditButton from "@/components/global/utilities/EditButton.vue";
import DeleteButton from "@/components/global/utilities/DeleteButton.vue";
import Form from "@/components/erp/productAndServices/category/SubCategory/Form.vue";
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("erp/ProductAndService/Category/SubCategory", {
  ...
  methods: {
    ...mapActions("erp/ProductAndService/Category/SubCategory", [
      "setUpsertResponse",
      "setUpsertError",
      "setDeleteResponse",
      "setDeleteError",
    ...
    selectAllRecords() {
      ...
      if (!this.selectAll) {
        for (let i in this.subCategories) {
          this.selected.push(this.subCategories[i]);
        }
      }
    },
    ////////// UPSERT ///////////////////
    ...
    openUpdate(item) {
      item["main_category"] = item.main_category_id;
    ...
    create() {
      this.$store
        .dispatch("erp/ProductAndService/Category/SubCategory/create", this.form)
        .then((res) => {
          this.resetForm();
          // console.log(res)
    ...
    update() {
      this.$store
        .dispatch("erp/ProductAndService/Category/SubCategory/update", this.form)
        .then((res) => {
          console.log(res)
          const summery = res.statusText;
    ...
    deleteRecords() {
      this.$store
        .dispatch("erp/ProductAndService/Category/SubCategory/delete", this.selected)
        .then((res) => {
    ...
    openDetailsDialog(record) {
      const { name, main_category_name, description, updated_at } = record;
      this.recordDetails = { name, main_category_name, description, updated_at };
      this.displayDetailsDialog = true;
    },
    list() {
      console.log("aklfknwjfw")
      this.$store.dispatch("erp/ProductAndService/Category/SubCategory/list", this.filterParams).then(() => {
