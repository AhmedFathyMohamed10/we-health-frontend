<template>
                ...
                <table class="table table-sm text-center">
                  ...
                  <tbody>
                    <tr v-for="(store, index) in stores" :key="index">
                      ...
                      <td class="align-middle fw-bold">
                        <span>
                        <span v-for="(employee,index) in store.employee_data" :key="index">
                            {{ employee.name }}   
                        </span>
                      </span>
                      </td>
                      ...
                      <td class="align-middle">
                        ...
                        >
                          <EditButton
                            class="me-2"
                            @edit="openUpdate(store)"
                          ></EditButton>
                          <DeleteButton
                            @delete="openDeleteDialogSingle(store.code, 4)"
                          ></DeleteButton>
                        </div>
                      </td>
                    </tr>
                  ...
                  <tr v-for="(value, key) in recordDetails" :key="key">
                    ...
                    <td class="">
                      ...
                      <span v-if="key == 'updated_at' ">{{
                       ...
                       <span v-if="key == 'employee'">
                        {{ value.employee }}
                        <span v-for="(employee,index) in value" :key="index">
                            {{ employee.name }}
                        </span>
                      </span>
                    </td>
  ...
  import Form from "@/components/erp/productAndServices/store/Form.vue";
  ...
  export default {
    ...
    computed: {
      ...
      ...mapGetters("erp/ProductAndService/Store", {
    ...
    methods: {
      ...mapActions("erp/ProductAndService/Store", [
        "setUpsertResponse",
        "setUpsertError",
        "setStatusResponse",
        "setStatusError",
      ...
      selectAllRecords() {
        ...
        if (!this.selectAll) {
          for (let i in this.stores) {
            this.selected.push(this.stores[i].code);
          }
        }
      },
      ...
      openUpdate(item) {
        let newItem = [];
        for(let i in item.employee_data){
          newItem.push(item.employee_data[i].code)
        }
        const {
            id,
            code,
      ...
      create() {
        this.$store
          .dispatch("erp/ProductAndService/Store/create", this.form)
          .then((res) => {
            this.resetForm();
      ...
      update() {
        this.$store
          .dispatch("erp/ProductAndService/Store/update", this.form)
          .then((res) => {
            this.resetForm();
            const summery = res.statusText;
      ...
      deleteRecords() {
        this.$store
          .dispatch("erp/ProductAndService/Store/changeStatus",  {"action":this.actionStatus,"selected":this.selected})
          .then((res) => {
      ...
      list() {
        this.$store.dispatch("erp/ProductAndService/Store/list", this.filterParams).then(() => {
