...
<template>
              ...
              <div class="col-12">
                <div class="table-responsive">
                  <table class="table table-sm text-center">
                    ...
                    <tbody>
                      <tr v-for="(Liability, index) in Liabilitiess" :key="index">
                        ...
                        <!-- <td class="align-middle">
                          <span>
                            <a
                              ...
                              @click="openDetailsDialog(Liability)"
                              >{{ Liability.employee_name }}</a
                            >
                          </span>
                        </td> -->
                        <td class="align-middle fw-bold">
                          <span>{{ Liability.employee_name }}</span>
                        </td>
                        <td class="align-middle fw-bold">
                            <span>{{ Liability.reason_name }}</span>
                          </td>
                        <td class="align-middle fw-bold">
                          <span>{{ Liability.amount }}</span>
                        </td>
                        <td class="align-middle fw-bold">
                            <span>{{ Liability.due_date }}</span>
                          </td>
                        ...
                        <td class="align-middle">
                          <div class="d-flex justify-content-center align-items-center">
                            <DetailButton
                            class="me-2"
                            @details="openDetailsDialog(Liability)"
                          ></DetailButton>
                            <EditButton class="me-2" @edit="openUpdate(Liability)"></EditButton>
                            <DeleteButton @delete="openDeleteDialogSingle(Liability.id)"></DeleteButton>
                          </div>
                        </td>  
                      </tr>
                    </tbody>
  ...
  <i18n src="@/lang/erp/Liabilities/Liabilities.json"></i18n>
  <script>
  ...
  import Form from '@/components/erp/HR/liabilities/Form.vue';
  ...
  export default {
    ...
    methods: {
      ...
      ...mapActions("erp/hr/liabilities",
      ["setUpsertResponse","setUpsertError","setDeleteResponse","setDeleteError"]),
      /////////////////////////////////////////
      selectAllRecords() {
        this.selected = [];
        if (!this.selectAll) {
          for (let i in this.Liabilitiess) {
            this.selected.push(this.Liabilitiess[i].id);
          }
        }
      },
      ////////// UPSERT ///////////////////
      ...
      create(){
        this.$store.dispatch("erp/hr/liabilities/create", this.form)
        .then((res) => {
          this.resetForm();
          // console.log(res)
      ...
      update(){
        this.$store.dispatch("erp/hr/liabilities/update", this.form)
        .then((res) => {
          this.resetForm();
          // console.log(res)
          const summery = res.statusText; 
      ...
      deleteRecords() {
        this.$store.dispatch("erp/hr/liabilities/delete", this.selected)
        .then((res) => {
      ...
      ///////////////// DETAILS /////////////////////////////
       ...
       openDetailsDialog(record) {
        ...
        const {amount,due_date,reason_name,employee_name,created_at} = record
        this.recordDetails = {amount,due_date,reason_name,employee_name,created_at};
       ...
       list() {
        ...
        if(this.isSearching){
         ...
         this.$store.dispatch("erp/hr/liabilities/list", params)
         .then(() => { 
