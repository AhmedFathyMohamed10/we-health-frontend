<template>
  <div class="Vacation">
    ...
    <Form v-model="form" :displayForm="displayForm" @close="resetForm" :error="upsertError" @cancel="resetForm" @save="upsert"  :header="isUpdating?$t('dialog.header.edit'):$t('dialog.header.new')"> 
    </Form>
...
export default {
  ...
  methods: {
    ...
    create(){
      this.$store.dispatch("erp/hr/vacations/create", this.form)
      .then((res) => {
        this.resetForm();
        // console.log(res)
    ...
    deleteRecords() {
      this.statusAction= 4
      this.$store.dispatch("erp/hr/vacations/delete", this.selected)
      .then((res) => {
    ...
    ///////////////// DETAILS /////////////////////////////
     ...
     list() {
       this.$store.dispatch("erp/hr/vacations/list", this.filterParams)
       .then(() => { 
