              ...
              <table class="table table-sm text-center">
                <thead>
                  <tr>
                    ...
                    <th scope="col">{{ $t("form.price.label") }}</th>
                  ...
                  <tr v-for="(course, index) in Courses" :key="index">
                    ...
                    <td class="align-middle">
                      <span>
                        <a
                          ...
                          @click="openDetailsDialog(course)"
                          >{{ course.name }}</a
                        >
                      </span>
                    </td>
                    <td class="align-middle fw-bold">
                      <span>{{ course.entity_name }}</span>
                    </td>
                    <td class="align-middle fw-bold">
                    ...
                    <td class="align-middle">
                      ...
                      >
                        <EditButton
                          class="me-2"
                          @edit="openUpdate(course)"
                        ></EditButton>
                        <DeleteButton
                          @delete="openDeleteDialogSingle(course.code,4)"
                        ></DeleteButton>
                      </div>
                    </td>
                  </tr>
              ...
              <tbody>
                <tr v-for="(value, key) in recordDetails" :key="key">
                  ...
                  <td class="">
                    ...
                    <span v-if="key!=='status' && key=='duration' ">{{value}} months</span>
                    <div v-if="key == 'status'">
                      <span v-if="value == '1'">
                        <Tag
                          severity="success"
...
// import Form "@/components/erp/HR/internships/courses/Form.vue"
...
export default {
  ...
  methods: {
    ...
    selectAllRecords() {
      ...
      if (!this.selectAll) {
        for (let i in this.Courses) {
          this.selected.push(this.Courses[i].code);
        }
      }
    },
    ////////// UPSERT ///////////////////
    ...
    create() {
      this.$store
        .dispatch("erp/hr/courses/create", this.form)
        .then((res) => {
          this.resetForm();
          // console.log(res)
    ...
    update() {
      this.$store
        .dispatch("erp/hr/courses/update", this.form)
        .then((res) => {
          this.resetForm();
          // console.log(res)
          const summery = res.statusText;
    ...
    openDeleteDialog() {
      ...
      deleteRecords() {
        this.$store.dispatch("erp/hr/courses/delete", {'selected':this.selected,'action':this.statusAction})
        .then((res) => {
          if (this.selected.length == this.pagination.rows){
            ...
            console.log(this.pagination.rows)
          })
          .catch((err) => {
            // console.log(err)
            const summery = err.statusText; 
    ...
    openDetailsDialog(record) {
      ...
      const { name,entity_name,duration,hours,course_type} = record
      this.recordDetails = { name,entity_name,duration,hours,course_type};
    ...
    list() {
      ...
      this.$store.dispatch("erp/hr/courses/list", params).then(() => {
