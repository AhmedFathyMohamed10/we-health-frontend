...
<template>
  <div class="Departments">
                    ...
                    <tr v-for="(department, index) in departments" :key="index">
                      ...
                      <td class="align-middle">
                        <span>
                          <a
                            ...
                            @click="openDetailsDialog(department)"
                            >{{ department.name }}</a
                          >
                        </span>
                      </td>
                      ...
                      <td class="align-middle">
                        <div class="d-flex justify-content-center align-items-center">
                          <EditButton class="me-2" @edit="openUpdate(department)"></EditButton>
                          <DeleteButton @delete="openDeleteDialogSingle(department.id)"></DeleteButton>
                        </div>
                      </td>  
                    </tr>
                  </tbody>
    ...
    <Form v-model="form" :isLoading="upsertLoading" :displayForm="displayForm" @close="resetForm" @cancel="resetForm" @save="upsert" :error="upsertError" :header="isUpdating?$t('dialog.title.edit'):$t('dialog.title.new')"> 
    </Form>
    <DeleteDialog
    ...
    <DetailsDialog :display="displayDetailsDialog" @hide="closeDetailsDialog" :header="$t('dialog.title.details')">
      <div class="row">
        <div class="col-md-12">
            <div class="table-responsive">
...
export default {
  ...
  methods: {
    ...
    selectAllRecords() {
      ...
      if (!this.selectAll) {
        for (let i in this.departments) {
          this.selected.push(this.departments[i].id);
        }
      }
    },
    ////////// UPSERT ///////////////////
    ...
    create(){
      this.$store.dispatch("erp/hr/departments/create", this.form)
      .then((res) => {
        this.resetForm();
        // console.log(res)
    ...
    update(){
      this.$store.dispatch("erp/hr/departments/update", this.form)
      .then((res) => {
        this.resetForm();
        // console.log(res)
        const summery = res.statusText; 
    ...
    deleteRecords() {
      this.$store.dispatch("erp/hr/departments/delete", this.selected)
      .then((res) => {
    ...
    ///////////////// DETAILS /////////////////////////////
     ...
     openDetailsDialog(record) {
      ...
      const {name,description,created_at} = record
      this.recordDetails = {name,description,created_at};
