<template>
    <div class="partner-ratios">
      ...
      <div class="search-wrapper my-2">
        <Base>
          <div class="row">
            <div class="col-12">
              <h4>{{ $t("partnerRatios.header") }}</h4>
            </div>
          </div>
          <div class="row">
            <div class="col-md-5 col-xxl-3">
              <div class="form-group">
                <label class="form-label"> {{ $t("form.partner.label") }} </label>
                <span
                  :class="[
                ...
                <table class="table table-sm text-center">
                  <thead>
                    <tr>
                      ...
                      <th scope="col">{{ $t("form.partner.label") }}</th>
                      <th scope="col">{{ $t("form.accountingPeriod.label") }}</th>
                      <th scope="col">{{ $t("form.ratio.label") }}</th>
                      <th scope="col">{{ $t("table.thead.controls") }}</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr v-for="(partnerRatio, index) in partnerRatios" :key="index">
                      <td class="align-middle">
                        <input
                          type="checkbox"
                          :value="partnerRatio.code"
                          v-model="selected"
                          :binary="true"
                        />
                      </td>
                      ...
                      <td class="align-middle">
                        <span>
                          <a
                            ...
                            @click="openDetailsDialog(partnerRatio)"
                            >
                            {{ partnerRatio.partner_data.name }}</a
                          >
                        </span>
                      </td>
                      <td class="align-middle fw-bold">
                        <span>{{ partnerRatio.accountingPeriod_data.name }}</span>
                      </td>
                      <td class="align-middle fw-bold">
                        <span v-if="partnerRatio.ratio != ''">
                        % {{ partnerRatio.ratio }}
                        </span>
                        </td>
                      <td class="align-middle">
                        ...
                        >
                          <EditButton
                            class="me-2"
                            @edit="openUpdate(partnerRatio)"
                          ></EditButton>
                          <DeleteButton
                            @delete="openDeleteDialogSingle(partnerRatio.code, 4)"
                          ></DeleteButton>
                        </div>
                      </td>
                    </tr>
              ...
              <table class="table table-borderless text-nowrap mb-0">
                <tbody>
                  <tr v-for="(value, key) in recordDetails" :key="key">
                    <td class="">
                      {{ $t(`partnerRatios.key.${key}`) }}
                    </td>
                    <td class="">
                      <span v-if="!value" class="text-muted">------</span>
                      <span
                        v-if="key != 'updated_at'&& key != 'ratio'"
                        >{{ value }}</span
                      >
                      <span v-if="key == 'ratio'&& value != ''">
                        % {{ value }}
                        </span>
  ...
  import Form from "@/components/erp/partnersAndRatios/partnerRatios/Form.vue";
  ...
  export default {
    ...
    computed: {
      ...
      ...mapGetters("erp/PartnersAndRatios/PartnerRatios", {
      ...
      filterParams() {
        return {
          ...
          partner: this.isSearching ? this.searchQuery.partner : "",
        };
      },
    },
    mounted() {
      if (!this.partnerRatios.length > 0) {
        this.list();
      }
    },
    methods: {
      ...mapActions("erp/PartnersAndRatios/PartnerRatios", [
        "setUpsertResponse",
        "setUpsertError",
        "setDeleteResponse",
        "setDeleteError",
      ...
      selectAllRecords() {
        this.selected = [];
        if (!this.selectAll) {
          for (let i in this.partnerRatios) {
            this.selected.push(this.partnerRatios[i].code);
          }
        }
      },
      ...
      openUpdate(item) {
         const {
          id,
          code,
          partner = item.partner_data.id,
          accountingPeriod = item.accountingPeriod_data.id,
          ratio,
          updated_at,
        } = item;
        this.form = {
      ...
      create() {
        this.$store
          .dispatch("erp/PartnersAndRatios/PartnerRatios/create", this.form)
          .then((res) => {
            this.resetForm();
      ...
      update() {
        this.$store
          .dispatch("erp/PartnersAndRatios/PartnerRatios/update", this.form)
          .then((res) => {
            this.resetForm();
            const summery = res.statusText;
      ...
      deleteRecords() {
        this.$store
          .dispatch("erp/PartnersAndRatios/PartnerRatios/changeStatus", {"action":this.actionStatus,"selected":this.selected})
          .then((res) => {
      ...
      openDetailsDialog(record) {
        const {
          partner = record.partner_data.name,
          accountingPeriod = record.accountingPeriod_data.name,
          ratio,
          updated_at,
        } = record;
        this.recordDetails = {
      ...
      list() {
        this.$store.dispatch("erp/PartnersAndRatios/PartnerRatios/list", this.filterParams).then(() => {
