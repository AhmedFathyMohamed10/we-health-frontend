      ...
      <Base>
        ...
        v-if="listLoading" 
          :columns="[
            ...
            $t('form.total_quantity_returns.label'),
            $t('form.total_price_returns.label'),
            $t('form.total_sales.label'),
            $t('form.total_profit.label'),
          ]" 
          :rows="5"
        />
        <div class="row" v-if="totalRecords && !listLoading">
          <div class="col-12">
            <div class="d-flex justify-content-between flex-wrap">
              <div><h4>{{ $t("productProfit.header") }}</h4></div>
              <div class="result">
              ...
              <table class="table table-sm text-center">
                <thead>
                  <tr>
                    ...
                    <th scope="col">{{ $t("form.total_quantity_returns.label") }}</th>
                    <th scope="col">{{ $t("form.total_price_returns.label") }}</th>
                    <th scope="col">{{ $t("form.total_sales.label") }}</th>
                    <th scope="col">{{ $t("form.total_profit.label") }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="(statement, index) in statements" :key="index">
                    ...
                    <td class="align-middle">
                      {{ statement.total_quantity_returns }}
                    </td>
                    <td class="align-middle">
                      {{ statement.total_price_returns }}
                    </td>
                    <td class="align-middle">
                      {{ statement.total_sales }}
                    </td>
                    <td class="align-middle">
...
<i18n src="@/lang/erp/accounting/balances/productProfit.json"></i18n>
<script>
...
export default {
  ...
  computed: {
    ...mapGetters("erp/accounting/balances/productProfit", {
  ...
  methods: {
    list() {
      this.$store.dispatch("erp/accounting/balances/productProfit/list", this.filterParams).then(() => {
