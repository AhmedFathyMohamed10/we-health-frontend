                ...
                <table class="table table-sm text-center">
                  ...
                  <tbody>
                    <tr v-for="(unit, index) in units" :key="index">
                      ...
                      <td class="align-middle">{{ unit.name_ar }}</td>
                      <td class="align-middle">
                        <div
...
<i18n src="@/lang/erp_v2/items/unit.json"></i18n>
<script>
...
export default {
  ...
  methods: {
    ...
    selectAllRecords() {
      ...
      if (!this.selectAll) {
        for (let i in this.units) {
          this.selected.push(this.units[i].code);
        }
      }
    },
    ...
    update() {
      this.$store
        .dispatch("erp_v2/items/unit/update", this.form)
        .then((res) => {
          this.resetForm();
          const summery = res.statusText;
    ...
    deleteRecords() {
      this.$store
        .dispatch("erp_v2/items/unit/changeStatus", {"action":this.actionStatus,"selected":this.selected})
        .then((res) => {
    ...
    list() {
      this.$store.dispatch("erp_v2/items/unit/list", this.filterParams).then(() => {
