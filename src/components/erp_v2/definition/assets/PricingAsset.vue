                      ...
                      <tr v-for="(price, index) in prices" :key="index">
                        ...
                        <td class="align-middle">
                          ...
                          >
                            <EditButton
                              class="me-2"
                              @edit="openUpdate(price)"
                            ></EditButton>
                            <DeleteButton
                              @delete="openDeleteDialogSingle(price.code, 4)"
                            ></DeleteButton>
                          </div>
                        </td>
                      </tr>
...
<i18n src="@/lang/erp_v2/assets/pricing.json"></i18n>
<script>
...
export default {
  ...
  methods: {
    ...
    selectAllRecords() {
      ...
      if (!this.selectAll) {
        for (let i in this.prices) {
          this.selected.push(this.prices[i].code);
        }
      }
    },
    ...
    openUpdate(item) {
      if(item['account_data']){
        item['account']=item['account_data']['account_name_en']
      }
      const {
        id,
        code,
    ...
    update() {
      this.$store
        .dispatch("erp_v2/assets/assetPrice/update", this.form)
        .then((res) => {
          this.resetForm();
          const summery = res.statusText;
    ...
    deleteRecords() {
      this.$store
        .dispatch("erp_v2/assets/assetPrice/delete", {"action":this.action,"selected":this.selected})
        .then((res) => {
    ...
    list() {
      this.$store.dispatch("erp_v2/assets/assetPrice/list", this.filterParams).then(() => {
