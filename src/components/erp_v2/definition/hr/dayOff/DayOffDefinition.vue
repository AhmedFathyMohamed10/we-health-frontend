  ...
  <div class="day-off-definition">
    <EditDialog 
          ...
          <div class="d-flex flex-wrap justify-content-between gap-2">
            <div class="d-flex gap-2 align-items-center">
              ...
              <span :class="[locale == 'ar' ? 'p-input-icon-left' : 'p-input-icon-right',]">
                ...
                <InputText
                  type="text"
                  v-model="searchQuery.day_off_name"
              ...
              <table class="table table-sm text-center">
                <thead>
                  <tr>
                    ...
                    <th scope="col">{{ $t("form.day_off_name.label") }}</th>
                    <th scope="col">{{ $t("form.day_off_type.label") }}</th>
                ...
                <tbody>
                  <tr v-for="(item, index) in daysOff" :key="index">
                    ...
                    <td class="align-middle">{{ findLocaleOption(day_off_options, item.day_off_type) }}</td>
                    <td class="align-middle">
                      <div
      ...
      <Form
        ...
        :day_off_options="day_off_options"
        @close="resetForm"
        @cancel="resetForm"
        @save="upsert"
        :error="upsertError"
              ...
              <tbody>
                <tr v-for="(value, key) in recordDetails" :key="key">
                  ...
                  <td class="">
                    ...
                    <span
                      v-if="key != 'updated_at' && key != 'day_off_type'"
                      >{{ value }}</span
                    >
                    <span v-if="key == 'updated_at'">{{
                      $moment(value).format("MMMM Do YYYY, h:mm:ss a")
                    }}</span>
                    <span v-if="key == 'day_off_type'">{{
                      findLocaleOption(day_off_options , value)
                    }}</span>
                  </td>
                </tr>
              </tbody>
...
import Form from "@/components/erp_v2/hr/dayOffDefinition/Form.vue";
import DetailsDialog from "@/components/global/utilities/DetailsDialog.vue";
import DeleteDialog from "@/components/global/utilities/DeleteDialog.vue";
export default {
  name: "day-off-definition",
  ...
  data() {
    return {
      ...
      day_off_options: [
        { en: "Salary Deduction", ar: "الخصم من المرتب", id: 1 },
        { en: "official holiday", ar: "عطلة رسمية", id: 2 },
        { en: "leave balance deduction", ar: "خصم رصيد الإجازة", id: 3 },
      ],
      isUpdating: false,
      ////////////////////
  ...
  computed: {
    ...
    ...mapGetters("erp_v2/hr/dayOffDefinition", {
    ...
    filterParams() {
      return {
        ...
        day_off_name: this.isSearching ? this.searchQuery.day_off_name : "",
      };
    },
  },
  // mounted() {
  ...
  // },
  methods: {
    ...mapActions("erp_v2/hr/dayOffDefinition", [
      "setUpsertResponse",
      "setUpsertError",
      "setDeleteResponse",
      "setDeleteError",
    ...
    create() {
      this.$store
        .dispatch("erp_v2/hr/dayOffDefinition/create", this.form)
        .then((res) => {
          this.resetForm();
    ...
    update() {
      this.$store
        .dispatch("erp_v2/hr/dayOffDefinition/update", this.form)
        .then((res) => {
          this.resetForm();
          const summery = res.statusText;
    ...
    deleteRecords() {
      this.$store
        .dispatch("erp_v2/hr/dayOffDefinition/changeStatus", {"action":this.actionStatus,"selected":this.selected})
        .then((res) => {
    ...
    list() {
      this.$store.dispatch("erp_v2/hr/dayOffDefinition/list", this.filterParams).then(() => {
    ...
    search() {
      ...
      if (this.filterParams.day_off_name) {
