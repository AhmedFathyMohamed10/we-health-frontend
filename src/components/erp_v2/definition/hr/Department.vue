  ...
  <div class="department">
    <EditDialog 
                  ...
                  <table class="table table-sm text-center">
                    ...
                    <tbody>
                      <tr v-for="(item, index) in departments" :key="index">
                        ...
                        <td class="align-middle">
                          <span>
                            <a class="fw-bold" href="javascript:void(0)" @click="openDetailsDialog(item)">{{item.department_name }}</a>
                          </span>
                        </td>
                        <td class="align-middle">{{ item.parent_name }}</td>
                        <td class="align-middle">{{ findLocaleOption(role_options , item.role) }}</td>
                        <td class="align-middle">
      ...
      <Form v-model="form" :isLoading="upsertLoading" :displayForm="displayForm" :isUpdating="isUpdating"
        :role_options="role_options" @close="resetForm" @cancel="resetForm" @save="upsert" :error="upsertError"
              ...
              <tbody>
                <tr v-for="(value, key) in recordDetails" :key="key">
                  ...
                  <td class="">
                    ...
                    <span v-if="key == 'role'">{{
                      findLocaleOption(role_options , value)
                    }}</span>
                  </td>
                </tr>
              </tbody>
...
export default {
  ...
  methods: {
    ...
    selectAllRecords() {
      ...
      if (!this.selectAll) {
        for (let i in this.departments) {
          this.selected.push(this.departments[i].code);
        }
      }
    },
    ...
    update() {
      this.$store
        .dispatch("erp_v2/hr/department/update", this.form)
        .then((res) => {
          this.resetForm();
          const summery = res.statusText;
    ...
    deleteRecords() {
      this.$store
        .dispatch("erp_v2/hr/department/changeStatus", { "action": this.actionStatus, "selected": this.selected })
        .then((res) => {
    ...
    openDetailsDialog(record) {
      record['parent']=record['parent_name'];
      const {
        department_name,
        parent,
        serial_code,
    ...
    list() {
      this.$store.dispatch("erp_v2/hr/department/list", this.filterParams).then(() => {
    ...
    search() {
      ...
      this.isSearch = true;
      if (this.filterParams.department_name) {
