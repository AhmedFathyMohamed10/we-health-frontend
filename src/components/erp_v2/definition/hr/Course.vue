                  ...
                  <table class="table table-sm text-center">
                    ...
                    <tbody>
                      <tr v-for="(item, index) in courses" :key="index">
                        ...
                        <td class="align-middle">{{ findLocaleOption(course_type_options, item.course_type) }}</td>
                        <td class="align-middle">{{ item.duration }}</td>
                        <td class="align-middle"> {{findLocaleOption(payment_type_options, item.payment_type) }}</td>
...
export default {
  ...
  methods: {
    ...
    update() {
      this.$store
        .dispatch("erp_v2/hr/courses/update", this.form)
        .then((res) => {
          this.resetForm();
          const summery = res.statusText;
    ...
    deleteRecords() {
      this.$store
        .dispatch("erp_v2/hr/courses/changeStatus", {"action":this.actionStatus,"selected":this.selected})
        .then((res) => {
    ...
    list() {
      this.$store.dispatch("erp_v2/hr/courses/list", this.filterParams).then(() => {
