<template>
  <div class="overtime-overdue-record">
    <EditDialog 
      :display="display" 
      @close="cancelForm"
      :min-width="'80'"
                  ...
                  <table class="table table-sm text-center">
                    <thead>
                      <tr>
                        ...
                        <th scope="col">{{ $t("form.overTime.label") }}</th>
                        <th scope="col">{{ $t("form.hours.label") }}</th>
                    ...
                    <tbody>
                      <tr v-for="(item, index) in overTimesAndOverDueRecord" :key="index">
                        ...
                        <td class="align-middle">{{ item.overtime_data.over_time_name }}</td>
                        <td class="align-middle">{{ item.hours }}</td>
...
export default {
  ...
  watch: {
    display() {
      if(this.display){
        if (!this.overTimesAndOverDueRecord.length > 0) {
          this.list();
        }
      }
    }
  ...
  computed: {
    ...
    ...mapGetters("erp_v2/hr/overTimeAndOverDueRecord", {
  ...
  // },
  methods: {
    ...mapActions("erp_v2/hr/overTimeAndOverDueRecord", [
      "setUpsertResponse",
      "setUpsertError",
      "setDeleteResponse",
      "setDeleteError",
    ...
    selectAllRecords() {
      this.selected = [];
      if (!this.selectAll) {
        for (let i in this.overTimesAndOverDueRecord) {
          this.selected.push(this.overTimesAndOverDueRecord[i].code);
        }
      }
    },
    ...
    openUpdate(item) {
      ...
      item.overtime = item['overtime_data']
      const {
        id,
        code,
        employee,
    ...
    create() {
      ...
      this.$store
        .dispatch("erp_v2/hr/overTimeAndOverDueRecord/create", this.form)
        .then((res) => {
          this.resetForm();
    ...
    update() {
      ...
      this.$store
        .dispatch("erp_v2/hr/overTimeAndOverDueRecord/update", this.form)
        .then((res) => {
          this.resetForm();
          const summery = res.statusText;
    ...
    deleteRecords() {
      this.$store
        .dispatch("erp_v2/hr/overTimeAndOverDueRecord/changeStatus", { "action": this.actionStatus, "selected": this.selected })
        .then((res) => {
    ...
    openDetailsDialog(record) {
      ...
      if(record['overtime_data']) {
      record.overtime = record['overtime_data'].over_time_name}
      const {
        employee,
        overtime,
        hours,
    ...
    list() {
      this.$store.dispatch("erp_v2/hr/overTimeAndOverDueRecord/list", this.filterParams).then(() => {
