          ...
          <div class="d-flex flex-wrap justify-content-between gap-2">
            <div class="d-flex gap-2 align-items-center">
              ...
              <span :class="[locale == 'ar' ? 'p-input-icon-left' : 'p-input-icon-right',]">
                ...
                <InputText
                  type="text"
                  v-model="searchQuery.allowance_name"
          ...
          <Base>
            <div v-if="listLoading">
              <DataTable :value="[{}, {}, {}]">
                ...
                <Column field="allowance_name" header="Allowance Name">
                  <template #body>
                    <Skeleton></Skeleton>
                  </template>
                </Column>
                <Column field="account" header="Allowance Account Code">
                  <template #body>
                    <Skeleton></Skeleton>
                  </template>
                </Column>
                  ...
                  <table class="table table-sm text-center">
                    <thead>
                      <tr>
                        ...
                        <th scope="col">{{ $t("form.allowance_name.label") }}</th>
                        <th scope="col">{{ $t("form.account.label") }}</th>
                        <th scope="col">{{ $t("table.thead.controls") }}</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr v-for="(item, index) in allowanceDefinition" :key="index">
...
export default {
  ...
  watch: {
    display() {
      if(this.display){
        if (!this.allowanceDefinition.length > 0) {
          this.list();
        }
      }
    }
  ...
  computed: {
    ...
    filterParams() {
      return {
        ...
        allowance_name: this.isSearching ? this.searchQuery.allowance_name : "",
      };
    },
  },
  // created() {
  ...
  methods: {
    ...
    selectAllRecords() {
      this.selected = [];
      if (!this.selectAll) {
        for (let i in this.allowanceDefinition) {
          this.selected.push(this.allowanceDefinition[i].code);
        }
      }
    },
    ...
    update() {
      this.$store
        .dispatch("erp_v2/hr/allowanceDefinition/update", this.form)
        .then((res) => {
          this.resetForm();
          const summery = res.statusText;
    ...
    deleteRecords() {
      this.$store
        .dispatch("erp_v2/hr/allowanceDefinition/changeStatus", {"action":this.actionStatus,"selected":this.selected})
        .then((res) => {
    ...
    list() {
      this.$store.dispatch("erp_v2/hr/allowanceDefinition/list", this.filterParams).then(() => {
    ...
    search() {
      ...
      if (this.filterParams.allowance_name) {
    ...
    fetchAll() {
      this.isSearch=false;
      this.searchQuery = { allowance_name: "" };
      this.list();
    },
    onPage(event) {
