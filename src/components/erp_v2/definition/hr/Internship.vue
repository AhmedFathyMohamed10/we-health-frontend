          ...
          <Base>
            <div v-if="listLoading">
              <DataTable :value="[{}, {}, {}]">
                ...
                <Column field="trainee" header="Trainee">
                  <template #body>
                    <Skeleton></Skeleton>
                  </template>
                </Column>
                  ...
                  <table class="table table-sm text-center">
                    <thead>
                      <tr>
                        ...
                        <th scope="col">{{ $t("form.trainee.label") }}</th>
                        <th scope="col">{{ $t("form.course.label") }}</th>
...
import Form from "@/components/erp_v2/hr/internship/Form.vue";
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("erp_v2/hr/internship", {
    ...
    filterParams() {
      return {
        ...
        trainee: this.isSearching ? this.searchQuery.trainee : "",
      };
    },
  },
  // mounted() {
  ...
  // },
  methods: {
    ...mapActions("erp_v2/hr/internship", [
      "setUpsertResponse",
      "setUpsertError",
      "setDeleteResponse",
      "setDeleteError",
    ...
    openUpdate(item) {
      item.trainee = item['trainee_data']
    ...
    create() {
      if (this.form.start_date) {
        this.form.start_date = this.$moment(this.form.start_date).format(
          "YYYY-MM-DD"
        );
      }    
      if (this.form.end_date) {
        this.form.end_date = this.$moment(this.form.end_date).format(
          "YYYY-MM-DD"
        );
      }
      this.$store
        .dispatch("erp_v2/hr/internship/create", this.form)
        .then((res) => {
          this.resetForm();
    ...
    update() {
      ...
      this.$store
        .dispatch("erp_v2/hr/internship/update", this.form)
        .then((res) => {
          this.resetForm();
          const summery = res.statusText;
    ...
    deleteRecords() {
      this.$store
        .dispatch("erp_v2/hr/internship/changeStatus", {"action":this.actionStatus,"selected":this.selected})
        .then((res) => {
    ...
    openDetailsDialog(record) {
      record.trainee = record['trainee_data'].name
      record.course = record['course_name']
      const {
        trainee,
        course,
        start_date,
    ...
    list() {
      this.$store.dispatch("erp_v2/hr/internship/list", this.filterParams).then(() => {
