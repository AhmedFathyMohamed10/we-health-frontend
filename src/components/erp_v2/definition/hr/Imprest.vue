          ...
          <div class="d-flex flex-wrap justify-content-between gap-2">
            <div class="d-flex gap-2 align-items-center">
              ...
              <span :class="[locale == 'ar' ? 'p-input-icon-left' : 'p-input-icon-right',]">
                ...
                <InputText
                  type="text"
                  v-model="searchQuery.imprest_reason"
          ...
          <Base>
            <div v-if="listLoading">
              <DataTable :value="[{}, {}, {}]">
                ...
                <Column field="Imprest Reason" header="Imprest Reason">
                <template #body>
                  <Skeleton></Skeleton>
                </template>
              </Column>
              ...
              <Column field="Imprest Amount" header="Imprest Amount">
                <template #body>
                  <Skeleton></Skeleton>
                </template>
              </Column>
                  ...
                  <table class="table table-sm text-center">
                    <thead>
                      <tr>
                        ...
                        <th scope="col">{{ $t("form.imprest_reason.label") }}</th>
                        <th scope="col">{{ $t("form.due_date.label") }}</th>
                        <th scope="col">{{ $t("form.imprest_amount.label") }}</th>
                        <th scope="col">{{ $t("form.employee.label") }}</th>
                        <th scope="col">{{ $t("table.thead.controls") }}</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr v-for="(item, index) in imprest" :key="index">
                        ...
                        <td class="align-middle">
                          <span>
                            <a
                              ...
                              >{{ item.imprest_reason }}</a
                            >
                          </span>
                        </td>
                        <td class="align-middle">{{ item.due_date }}</td>
                        <td class="align-middle">{{ item.imprest_amount }}</td>

                        <td class="align-middle">{{ item.employee_data.name}}</td>
                        <td class="align-middle">
                          <div
                ...
                <tbody>
                  <tr v-for="(value, key) in recordDetails" :key="key">
                    ...
                    <td class="">
                      ...
                      findLocaleOption(course_type_options, value)
                      }}</span>
                      <span v-if="key == 'payment_type'">{{
                          findLocaleOption(payment_type_options, value)
                      }}</span>
                    </td>
                  </tr>
                </tbody>
...
import Form from "@/components/erp_v2/hr/imprest/Form.vue";
...
export default {
  ...
  computed: {
    ...
    filterParams() {
      return {
        ...
        imprest_reason: this.isSearching ? this.searchQuery.imprest_reason : "",
      };
    },
  },
  // mounted() {
  ...
  methods: {
    ...
    selectAllRecords() {
      this.selected = [];
      if (!this.selectAll) {
        for (let i in this.imprest) {
          this.selected.push(this.imprest[i].code);
        }
      }
    },
    ...
    create() {
      if (this.form.due_date) {
        this.form.due_date = this.$moment(this.form.due_date).format("YYYY-MM-DD");
      }
      this.$store
        .dispatch("erp_v2/hr/imprest/create", this.form)
        .then((res) => {
          this.resetForm();
    ...
    update() {
      this.$store
        .dispatch("erp_v2/hr/imprest/update", this.form)
        .then((res) => {
          this.resetForm();
          const summery = res.statusText;
    ...
    deleteRecords() {
      this.$store
        .dispatch("erp_v2/hr/imprest/changeStatus", {"action":this.actionStatus,"selected":this.selected})
        .then((res) => {
    ...
    list() {
      this.$store.dispatch("erp_v2/hr/imprest/list", this.filterParams).then(() => {
    ...
    search() {
      ...
      this.isSearch = true;
      if (this.filterParams.imprest_reason) {
    ...
    fetchAll() {
      this.isSearch = false;
      this.searchQuery = { imprest_reason: "" };
      this.list();
    },
    onPage(event) {
