                  ...
                  <table class="table table-sm text-center">
                    <thead>
                      <tr>
                        ...
                        <th scope="col">{{ $t("form.role_name.label") }}</th>
                        <th scope="col">{{ $t("form.department.label") }}</th>
...
import Form from "@/components/erp_v2/hr/role/Form.vue";
...
export default {
  ...
  computed: {
    ...
    filterParams() {
      return {
        ...
        role_name: this.isSearching ? this.searchQuery.role_name : "",
      };
    },
  },
  // mounted() {
  ...
  methods: {
    ...
    selectAllRecords() {
      ...
      if (!this.selectAll) {
        for (let i in this.roles) {
          this.selected.push(this.roles[i].code);
        }
      }
    },
    cancelForm(){
    ...
    openUpdate(item) {
      item['department']=item.department_data
       const {
        id,
        code,
        role_name,
    ...
    create() {
      this.$store
        .dispatch("erp_v2/hr/roles/create", this.form)
        .then((res) => {
          this.resetForm();
    ...
    update() {
      this.$store
        .dispatch("erp_v2/hr/roles/update", this.form)
        .then((res) => {
          this.resetForm();
          const summery = res.statusText;
    ...
    deleteRecords() {
      this.$store
        .dispatch("erp_v2/hr/roles/changeStatus", {"action":this.actionStatus,"selected":this.selected})
        .then((res) => {
    ...
    openDetailsDialog(record) {
      record['department']=record['role_name'];
      const {
        role_name,
        department,
        serial_code,
    ...
    list() {
      this.$store.dispatch("erp_v2/hr/roles/list", this.filterParams).then(() => {
    ...
    search() {
      ...
      this.isSearch = true;
      if (this.filterParams.role_name) {
