        ...
        <div class="d-flex flex-wrap justify-content-between gap-2">
          <div class="d-flex gap-2 align-items-center">
            ...
            <span :class="[locale == 'ar' ? 'p-input-icon-left' : 'p-input-icon-right',]">
              ...
              <InputText
                type="text"
                v-model="searchQuery.reward_name"
              ...
              <table class="table table-sm text-center">
                <thead>
                  <tr>
                    ...
                    <th scope="col">{{ $t("form.reward_name.label") }}</th>
                    <th scope="col">{{ $t("form.reward_code.label") }}</th>
                    <th scope="col">{{ $t("table.thead.controls") }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="(item, index) in paymentTypes" :key="index">
                    ...
                    <td class="align-middle">{{ item.reward_code }}</td>
                    <td class="align-middle">
                      <div
...
<i18n src="@/lang/erp_v2/invoices/paymentType.json"></i18n>
<script>
...
computed: {
  ...
  ...mapGetters("erp_v2/invoices/paymentType", {
  ...
  filterParams() {
    return {
      ...
      reward_name: this.isSearching ? this.searchQuery.reward_name : "",
    };
  },
},
// mounted() {
...
// },
methods: {
  ...mapActions("erp_v2/invoices/paymentType", [
    "setUpsertResponse",
    "setUpsertError",
    "setDeleteResponse",
    "setDeleteError",
  ...
  selectAllRecords() {
    ...
    if (!this.selectAll) {
      for (let i in this.paymentTypes) {
        this.selected.push(this.paymentTypes[i].code);
      }
    }
  },
  ...
  create() {
    this.$store
      .dispatch("erp_v2/invoices/paymentType/create", this.form)
      .then((res) => {
        this.resetForm();
  ...
  update() {
    this.$store
      .dispatch("erp_v2/invoices/paymentType/update", this.form)
      .then((res) => {
        this.resetForm();
        const summery = res.statusText;
  ...
  deleteRecords() {
    this.$store
      .dispatch("erp_v2/invoices/paymentType/changeStatus", {"action":this.actionStatus,"selected":this.selected})
      .then((res) => {
  ...
  list() {
    this.$store.dispatch("erp_v2/invoices/paymentType/list", this.filterParams).then(() => {
  ...
  search() {
    ...
    this.isSearch = true;
    if (this.filterParams.reward_name) {
